...
class VerifyRef():
    ...
    def extractReferences(self):
        ...
        for para in self.paragraphs:
            ...
            if self.verifyOptionalSections(para.text.lower().strip()):
                getRefs = False
            if getRefs and para.text:
                self.references.append(para.text)
            if self.verifyRefSection(para.text.lower().strip()):
                getRefs = True
